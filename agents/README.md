...
将 `agents/AGENTS.md` 复制到 reasonix、zcode 的全局配置目录,链接到 codex、opencode 和 deepseek-harness 的全局配置目录;已有目标直接覆盖。deepseek-harness 的全局指令固定在 `$DSH_HOME/AGENTS.md`(`DSH_HOME` 未设置时默认为 `~/.dsh/AGENTS.md`),脚本按该路径软链接。
